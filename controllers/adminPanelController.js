...
const addNewService = async (req, res) => {
  ...
  const imageUrl = `${process.env.FRONTEND_URL || "https://shahrpitaj.ir"}/uploads/${req.file.filename}`;
...
const addNewServicer = async (req, res) => {
  const imageUrl = req.file
    ? `${process.env.FRONTEND_URL || "https://shahrpitaj.ir"}/uploads/${req.file.filename}`
    : "";
...
const setPortfolioImages = async (req, res) => {
  ...
  const imagesUrl = files.map((file) => {
    return `${process.env.FRONTEND_URL}${process.env.PORT}/uploads/${file.filename}`;
  });
